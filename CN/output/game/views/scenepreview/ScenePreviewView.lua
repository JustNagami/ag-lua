		...
		if arg_11_0.curPage_ == 1 then
			...
			if not var_22_1 or not HeroTools.GetHeroIsUnlock(var_22_1) then
				if CharactorParamCfg[var_22_1] then
					ShowTips(GetTipsF("HOME_SCENE_UNAVAILABLE", CharactorParamCfg[var_22_1].Name))
				end
		...
		if var_39_3 == 0 then
			...
			if not var_39_6 or not HeroTools.GetHeroIsUnlock(var_39_6) then
				if CharactorParamCfg[var_39_6] then
					arg_39_0.banText_.text = GetTipsF("HOME_SCENE_UNAVAILABLE", CharactorParamCfg[var_39_6].Name)
				end
...
function var_0_0.BindRedpoint(arg_58_0)
	manager.redPoint:bindUIandKey(arg_58_0.sceneBtn_.transform, RedPointConst.SCENE_NORMAL)
	manager.redPoint:bindUIandKey(arg_58_0.dlcBtn_.transform, RedPointConst.SCENE_DLC)
end

function var_0_0.UnbindRedpoint(arg_59_0)
	manager.redPoint:unbindUIandKey(arg_59_0.sceneBtn_.transform)
	manager.redPoint:unbindUIandKey(arg_59_0.dlcBtn_.transform)
end

function var_0_0.Dispose(arg_60_0)
	arg_60_0:RemoveAllListeners()

	if arg_60_0.sceneScrollHelper_ then
		arg_60_0.sceneScrollHelper_:Dispose()

		arg_60_0.sceneScrollHelper_ = nil
	end

	if arg_60_0.dlcScrollHelper_ then
		arg_60_0.dlcScrollHelper_:Dispose()

		arg_60_0.dlcScrollHelper_ = nil
	end
