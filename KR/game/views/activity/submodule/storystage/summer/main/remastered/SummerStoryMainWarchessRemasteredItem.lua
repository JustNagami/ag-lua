...
function var_0_0.AddListeners(arg_3_0)
	arg_3_0:AddBtnListener(arg_3_0.button_, nil, function()
		if arg_3_0:IsPrestageLock() then
			...
			ShowTips(string.format(GetTips("UNLOCK_CONDITION_ACTIVITY_TIPS"), GetI18NText(GetTips("CHESS_NAME_" .. var_4_1.tag)), GetI18NText(WarchessLevelCfg[var_4_2].name_level)))
		...
		if arg_3_0:IsLock() then
			...
			if var_4_3[1] == 1 then
				ShowTips(string.format(GetTips("UNLOCK_CONDITION_ACTIVITY_TIPS"), GetI18NText(ActivityStoryChapterCfg[var_4_3[2][1]].name), GetI18NText(BattleActivityStoryStageCfg[var_4_3[2][2]].name)))
			elseif var_4_3[1] == 2 then
				ShowTips(string.format(GetTips("UNLOCK_CONDITION_ACTIVITY_TIPS"), GetI18NText(ChapterCfg[var_4_3[2][1]].subhead), GetI18NText(BattleActivityStoryStageCfg[var_4_3[2][2]].name)))
			end
